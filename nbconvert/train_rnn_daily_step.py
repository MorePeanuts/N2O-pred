...
with open(data_dir / 'sequences_daily_step_test_processed.pkl', 'rb') as f:
...
# Sample validation sequences for SHAP (use smaller sample for speed)
...
fert_numeric_names = ['Split N amount (avg)', 'ferdur (avg)'][:n_fert_numeric]
...
for idx in val_sample_indices:
    ...
    if obs_mask.sum() > 0:
        ...
        avg_fert_cat_fert = seq['fertilization_categorical_encoded']['fertilization_class'].mean(
            axis=0
        )
...
ax.set_title('SHAP Feature Importance (RNN-DailyStep)', fontsize=12)
ax.invert_yaxis()
